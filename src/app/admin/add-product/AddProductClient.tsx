...
const AddProductClient = () => {
    ...
    // firebase storage에 상품 이미지 저장
    const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
        if (!e.target.files) return;
        const file = e.target.files[0];

        // uploading image to fierebase
        // firebase storage에 저장(저장할 스토리지, 스토리지 경로)
    ...
    // firebase store에 상품 데이터 저장
    const addProduct = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setIsLoading(true);

        try {
            // Add a new document with a generaced id.
